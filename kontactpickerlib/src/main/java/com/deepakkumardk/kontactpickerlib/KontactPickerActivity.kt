...
class KontactPickerActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        kontactsAdapter = KontactsAdapter { contact, _, view ->
    ...
    private fun filterContacts(text: String) {
        ...
        if (text.isEmpty()) {
            tempList.clear()
            tempList.addAll(myKontacts)
        } else {
            for (contact in myKontacts) {
                val name = contact.contactName
                val number = contact.contactNumber
                if (name!!.startsWith(text, true) || number?.startsWith(text) == true) {
                    tempList.add(contact)
                }
            }
            myKontacts.forEach {
                if (!tempList.contains(it) && it.contactName!!.contains(text)) {
                    tempList.add(it)
                }
            }
        }
